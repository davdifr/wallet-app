...
export function DailyBudgetCard({ result, totalWealth }: DailyBudgetCardProps) {
  ...
  return (
    <Card className="overflow-hidden border-white/70 bg-white/85 shadow-soft backdrop-blur">
      <CardHeader className="space-y-5 bg-gradient-to-br from-slate-950 via-slate-900 to-slate-800 text-white">
      ...
      <CardContent className="space-y-5 p-5 sm:p-6">
        <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-3">
          <div className="rounded-3xl border border-slate-200 bg-slate-50/80 px-4 py-4">
            <p className="text-xs uppercase tracking-[0.18em] text-slate-500">Patrimonio Totale</p>
            <p className="mt-2 text-2xl font-semibold text-slate-950">{totalWealth}</p>
          </div>
          <div className="rounded-3xl border border-slate-200 bg-slate-50/80 px-4 py-4">
            <p className="text-xs uppercase tracking-[0.18em] text-slate-500">Nel Salvadanaio</p>
            <p className="mt-2 text-2xl font-semibold text-slate-950">
              {formatCurrency(result.blockedInPiggyBank)}
            </p>
          </div>
          <div className="rounded-3xl border border-slate-200 bg-slate-950 px-4 py-4 text-white">
            <p className="text-xs uppercase tracking-[0.18em] text-slate-300">
              Spendibile Residuo Mese
            </p>
          ...
          <div className="rounded-3xl border border-slate-200 bg-slate-50/80 px-4 py-4">
            <p className="text-xs uppercase tracking-[0.18em] text-slate-500">Giorni Rimanenti</p>
          ...
          <div className="rounded-3xl border border-slate-200 bg-slate-50/70 px-4 py-4">
            ...
            {warnings.length === 0 ? (
              <p className="mt-3 text-sm text-slate-600">
                Nessun warning: il ritmo attuale e coerente con il piano di fine mese.
              </p>
            ) : (
